...
while read FILE; do
    ...
    ## compile the blast results for each file
    JOB_ID3=`qsub -v SCRIPT_DIR,BLAST_OUT_DIR,FILE -W depend=afterok:$JOB_ID2 -e "$STDERR_DIR" -o "$STDOUT_DIR" $SCRIPT_DIR/run-compile-blast.sh`
